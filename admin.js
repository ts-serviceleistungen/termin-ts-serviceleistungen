...
async function loadReceipts(){
  ...
  // Einnahmen/Gewinn hängen von den aktuellen Belegdaten ab.
...
async function loadFinancials(){
  ...
  try{
    const response = await fetch(`${RECHNUNGS_API_URL}?v=${year}`);
...
refreshBtn.addEventListener('click',async()=>{
  ...
  try{
    await Promise.all([load(),loadReceipts(),loadFinancials()]);
  }finally{
