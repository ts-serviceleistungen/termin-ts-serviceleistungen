...
function renderRequests() {
  ...
  listEl.innerHTML = filtered.map((x) => {
    ...
    const middle = isGarden
      ...
      : `<div><b>${escapeHtml([x.make, x.model].filter(Boolean).join(' ') || x.vehicle_type || 'Fahrzeug')}</b><small>${escapeHtml(x.vehicle_type || '')} · ${escapeHtml(x.color || '')}</small></div>`;
    const duration = x.duration_label ? ` · ${escapeHtml(x.duration_label)}` : '';
    return `<div class="row request-row">
      <div><b>${escapeHtml(x.first_name)} ${escapeHtml(x.last_name)}</b><small>${escapeHtml(x.phone)}<br>${escapeHtml(x.email)}</small></div>
      ${middle}
      <div><b>${escapeHtml(x.service_type)}</b><small>${formatDate(x.requested_date)} · ${escapeHtml(x.requested_time || '—')}${duration}</small></div>
      <div><span class="badge status-${escapeHtml(normalizeStatus(x.status).replaceAll(' ', '-'))}">${escapeHtml(normalizeStatus(x.status))}</span><br>
...
function renderCalendar() {
  ...
  requests.forEach((r) => {
    ...
    counts[r.requested_date] = (counts[r.requested_date] || 0) + 1;
    const type = r.service_type === 'Gartenarbeiten' ? 'garden' : (r.service_type === 'Fahrzeugpflege' ? 'vehicle' : 'other');
    dayTypes[r.requested_date] = dayTypes[r.requested_date] || new Set();
    dayTypes[r.requested_date].add(type);
  });
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const types = dayTypes[key] ? [...dayTypes[key]] : [];
    const markers = types.map(type => {
      if (type === 'garden') return '<span title="Gartenarbeiten" style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#39a852;margin:2px"></span>';
      if (type === 'vehicle') return '<span title="Fahrzeugpflege" style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#e58bb1;margin:2px"></span>';
      return '<span title="Sonstiger Termin" style="display:inline-block;width:9px;height:9px;border-radius:50%;background:#999;margin:2px"></span>';
    }).join('');
    html += `
      <button type="button" class="${classes.join(' ')}" data-calendar-date="${key}">
        <span>${day}</span>
        ${count ? `<b>${count}</b>` : ''}
        ${markers ? `<span style="display:block;line-height:10px">${markers}</span>` : ''}
      </button>
    `;
  }
...
async function openDetails(id) {
  ...
  detailBody.innerHTML = `
    <div class="detail-grid">
      ...
      ${x.service_type === 'Gartenarbeiten' ? field('Gartenfläche', x.garden_area) : ''}
      ${x.service_type === 'Gartenarbeiten' ? field('Gartenzustand', x.garden_condition) : ''}
      ${field('Geplante Dauer', x.duration_label)}
    </div>
...
function calculateCalendarDuration(serviceType, careOptions) {
  const s = String(serviceType || '').toLowerCase();
  const options = Array.isArray(careOptions) ? careOptions.map(x => String(x).toLowerCase()) : [];
  if (s.includes('garten')) return Math.max(4, options.length * 4);
...
async function syncConfirmedCalendarEvent(previous, updated) {
  const previousCustomer = `${previous?.first_name || ''} ${previous?.last_name || ''}`.trim();
  const previousTitle = `Kundentermin – ${previousCustomer || 'Termin'} – ${previous?.service_type || 'Leistung'}`;
  const oldStart = previous?.requested_date && previous?.requested_time
    ? new Date(`${previous.requested_date}T${String(previous.requested_time).slice(0,8)}`).toISOString()
    : null;
  const oldHours = previous ? calculateCalendarDuration(previous.service_type, previous.care_options) : 2;
  const oldEnd = oldStart ? new Date(new Date(oldStart).getTime() + oldHours * 60 * 60 * 1000).toISOString() : null;

  // Alte Spiegelung entfernen, z. B. nach einer Terminänderung oder Ablehnung.
  if (oldStart && oldEnd) {
    await db.from('personal_calendar_events')
      .delete()
  ...
  if (String(updated?.status || '') !== 'Bestätigt' || !updated?.requested_date || !updated?.requested_time) return;

  const customer = `${updated.first_name || ''} ${updated.last_name || ''}`.trim();
  const title = `Kundentermin – ${customer || 'Termin'} – ${updated.service_type || 'Leistung'}`;
  const start = new Date(`${updated.requested_date}T${String(updated.requested_time).slice(0,8)}`);
  const hours = calculateCalendarDuration(updated.service_type, updated.care_options);
  const end = new Date(start.getTime() + hours * 60 * 60 * 1000);
  const startIso = start.toISOString();
  const endIso = end.toISOString();
  const eventType = updated.service_type === 'Gartenarbeiten' ? 'garden' : (updated.service_type === 'Fahrzeugpflege' ? 'vehicle' : 'other');

  const { data: existingSync, error: syncLookupError } = await db
  ...
  if (syncLookupError) {
    console.warn('Kalender-Synchronisierung:', syncLookupError);
    return;
  }

  if (!existingSync?.length) {
    const { error: syncError } = await db.from('personal_calendar_events').insert({
      title,
      event_type: eventType,
      start_time: startIso,
      end_time: endIso
    });
    if (syncError) console.warn('Kalender-Synchronisierung:', syncError);
  }
}
...
async function updateRequest(id, changes, emailStatus) {
  ...
  await syncConfirmedCalendarEvent(current, updated);
...
async function deleteRequest(id) {
  ...
  await syncConfirmedCalendarEvent(r, { ...r, status: 'Abgelehnt' });
